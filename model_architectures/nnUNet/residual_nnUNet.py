...
class ResNetBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        res = x
        out = self.conv(x)
        out = F.leaky_relu(self.in1(out), negative_slope = 0.03)

        return (out + res)
        

class ResnnUNetModel(nn.Module):
    ...
    def __init__(self, input_channels = 32, output_channels = 256, kernel_size = 3, **conv_kwargs):
        ...
        self.conv5 = nn.Conv3d(in_channels = 320, 
                               out_channels = 320, 
                               kernel_size = 3, 
                               stride = 2, 
                               padding = 1) 
        ...
        #self.cs_block1 = nn.Softmax(dim = 1)
        ...
        #self.cs_block2 = nn.Softmax(dim = 1)
        ...
        #self.cs_block3 = nn.Softmax(dim = 1)
        ...
        self.fconv.bias.data.fill_(0.01)
        #self.fnorm = nn.Softmax(dim = 1)
